...
This update turns the Telegram `/panel` surface into a PMBOT operator console for the accepted 052-059 paper and preflight flows. The console is review-only and exposes only status reads plus existing dry-run/preflight commands.
...
The run buttons execute the existing module commands with `--dry-run` and no live/auth/signing/order/wallet enabling flags. The status buttons only render Telegram-safe summaries.
...
- Signer boundary: not implemented yet
