...
def render_dashboard_page(data: dict[str, Any], *, repo_root: str, queue_root: str) -> str:
    ...
    nightly_lane_batch = data.get("nightly_lane_batch", {})
...
<section class="grid">
  ...
  <div class="card"><strong>Nightly batch</strong><br><span class="{_status_class('ok' if nightly_lane_batch.get('ready') else nightly_lane_batch.get('status', 'missing'))}">{_e(nightly_lane_batch.get('status', 'missing'))}</span><br>tasks {_e(nightly_lane_batch.get('task_count', 0))}, done {_e(nightly_lane_batch.get('completed_count', 0))}, blocked {_e(nightly_lane_batch.get('blocked_count', 0))}, failed {_e(nightly_lane_batch.get('failed_count', 0))}<br><code>{_e(nightly_lane_batch.get('latest_report_json', 'none'))}</code></div>
...
<section class="grid">
  <div class="card"><strong>Subagent profiles</strong><br><code>{_e(project_contract.get('subagent_profiles_path', 'agent_tasks/agents/'))}</code></div>
  <div class="card"><strong>Maintenance prompt</strong><br><code>{_e(project_contract.get('maintenance_prompt_path', 'agent_tasks/automations/codex_maintenance_prompt.md'))}</code></div>
</section>
<section class="card">
  <h2>Plan runner</h2>
...
def _status_class(status: Any) -> str:
    ...
    if value in {"paused", "recovering", "needs_retry", "max_steps"}:
        return "warn"
    if value in {"completed", "done", "dry_run", "recovered", "running", "ok"}:
        return "ok"
    return ""
