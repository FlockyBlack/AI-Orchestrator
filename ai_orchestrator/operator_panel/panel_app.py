...
def route_get(path: str, query: str, repo_root: str | Path, queue_root: str | Path) -> str:
    ...
    if path == "/run":
        ...
        run = next((item for item in discover_runs(queue_root) if item["run_id"] == run_id), {"run_id": run_id, "status": "missing"})
        return render_run_page(run, _collect_artifacts(queue_root, run_id), repo_root=str(repo_root), queue_root=str(queue_root))
    if path == "/artifacts":
        return render_artifacts_page(_collect_artifacts(queue_root), repo_root=str(repo_root), queue_root=str(queue_root))
    if path == "/git":
    ...
    if path == "/codex-handoff":
        ...
        return render_handoff_page(prompt, paths, repo_root=str(repo_root), queue_root=str(queue_root))
...
def route_post(path: str, form: dict[str, str], repo_root: str | Path, queue_root: str | Path) -> dict[str, Any]:
    ...
    if path == "/actions/continue-run":
        return panel_api.post_continue_run(form, queue_root)
    if path == "/actions/recover-run":
        return panel_api.post_recover_run(form, queue_root)
    if path == "/actions/export-codex-prompt":
        return panel_api.post_export_handoff_prompt(form, queue_root)
