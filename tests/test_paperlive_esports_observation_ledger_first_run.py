...
from pm_bot.paper_live import esports_observation_ledger_first_run as first_run  # noqa: E402


MODULE_PATH = (
    ROOT / "pm_bot" / "paper_live" / "esports_observation_ledger_first_run.py"
)
...
def _prepare_root(root):
    for relative_path in [
        ...
        first_run.LEDGER_CONTRACT_MD_PATH,
...
def test_write_creates_observation_ledger_entry_for_1987056(tmp_path):
    _prepare_root(tmp_path)
    ...
    assert ledger["market_id"] == "1987056"
    assert ledger["title_or_question"] == first_run.MARKET_TITLE


def test_ledger_entry_is_observation_only_and_has_no_trade_fields_set(tmp_path):
    _prepare_root(tmp_path)
    first_run.write_artifacts(tmp_path)

    ledger = _load_json(tmp_path / first_run.LEDGER_ENTRY_JSON_PATH)

    assert ledger["observation_mode"] == "source_and_outcome_tracking_only"
...
def test_ledger_entry_has_required_observation_sections(tmp_path):
    _prepare_root(tmp_path)
    first_run.write_artifacts(tmp_path)
    ...
    assert "match_identity" in monitored_ids
    assert "game_title" in monitored_ids
    assert "tournament_identity" in monitored_ids
    assert "teams_or_players_identity" in monitored_ids
    assert "match_format" in monitored_ids
    assert "official_result_source" in monitored_ids
    assert "scheduled_time_timezone" in monitored_ids
    assert "cancellation_reschedule_forfeit_handling" in monitored_ids
    assert "final_match_result" in monitored_ids
    assert "polymarket_exact_rules_or_description" in monitored_ids
    assert "market_specific_rules_text_complete" in monitored_ids
    assert "market_metadata_source" in source_roles
    assert "market_rules_source" in source_roles
    assert "official_result_source_candidate" in source_roles
    assert "fallback_credible_result_source" in source_roles
...
def test_source_quality_pending_observation_roles_are_allowed(tmp_path):
    ...
    assert set(observation["source_roles_observed"]).issubset(allowed_roles)
...
def test_outcome_reconciliation_placeholder_exists_and_outcome_unknown(tmp_path):
    ...
    assert placeholder["official_result_source_required"] is True
    assert (
...
def test_run_summary_preserves_source_009b_009c_counts(tmp_path):
    _prepare_root(tmp_path)
    first_run.write_artifacts(tmp_path)
...
def test_no_forbidden_source_quality_or_market_decision_keys_in_outputs(tmp_path):
    ...
    for relative_path in JSON_OUTPUT_PATHS:
        ...
        assert forbidden_keys.isdisjoint(keys), relative_path
