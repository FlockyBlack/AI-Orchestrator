...
CONTRACT_VERSION = "llm_analysis_review_stub_contract.v1"
REVIEW_STUB_VERSION = "offline_llm_review_stub.v1"
DETERMINISTIC_GENERATED_AT = "deterministic-offline-review-stub.v1"

DEFAULT_PACKET_PATH = validator.DEFAULT_PACKET_PATH
DEFAULT_RESPONSE_PATH = validator.DEFAULT_RESPONSE_PATH
DEFAULT_OUT_JSON_PATH = validator.LLM_DIR / "offline_llm_review_stub.v1.json"
DEFAULT_OUT_MD_PATH = validator.LLM_DIR / "offline_llm_review_stub.v1.md"


def _parse_args(argv):
    parser = argparse.ArgumentParser(
        description="Export a deterministic offline PMBOT LLM review stub from local JSON artifacts."
    )
...
def _allowed_response_sections_present(response_payload):
...
def _source_artifacts(packet_payload):
    declared = []
...
def _operator_summary(status):
    if status == "accepted":
        return (
            "Accepted: the packet and response passed PMBOT-LLM-001 offline validation. "
            "Use the response sections only as a manual review aid; this stub is not trading advice "
            "and does not make market decisions."
        )
    return (
        "Rejected: the packet or response failed PMBOT-LLM-001 offline validation. "
...
def _next_safe_operator_action(status):
    if status == "accepted":
        return "Review the accepted sections against local source artifacts and record unresolved evidence gaps manually."
    return "Inspect the listed validation errors, edit only local packet or response JSON, and rerun this offline exporter."


def build_review_stub(packet_path=DEFAULT_PACKET_PATH, response_path=DEFAULT_RESPONSE_PATH):
    ...
    return {
        ...
        "review_stub_version": REVIEW_STUB_VERSION,
        ...
        "allowed_response_sections_present": _allowed_response_sections_present(response_payload),
        ...
        "source_artifacts": _source_artifacts(packet_payload),
    }
...
def render_markdown_summary(result):
    ...
    lines = [
        "# PMBOT Offline LLM Review Stub v1",
        "",
        ...
        *_format_messages(result["warnings"]),
        "",
        "## Allowed LLM Sections Found",
        *_format_section_list(result["allowed_response_sections_present"]),
        "",
        ...
        *_format_messages(forbidden_findings),
        ...
        "## Boundary Notice",
        (
            "This is an offline stub only and not trading advice. It does not add API calls, LLM API calls, "
            "runtime integration, orders, autonomous paper orders, probability estimates, value scoring, "
            "side selection, or market decision logic."
        ),
        "",
    ]
    return "\n".join(lines)
...
def export_review_stub(
    ...
    result = build_review_stub(packet_path, response_path)
...
def main(argv):
    ...
    result = export_review_stub(args.packet, args.response, args.out_json, args.out_md)
