...
RUNNER = ROOT / "pm_bot" / "export" / "build_review_export_package.py"
EXPECTED_JSON = ROOT / "pm_bot" / "export" / "expected_review_export_package.v1.json"
EXPECTED_MD = ROOT / "pm_bot" / "export" / "expected_review_export_package.v1.md"
...
class ReviewExportPackageTests(unittest.TestCase):
    ...
    def test_package_keeps_review_only_scope(self):
        payload = json.loads(_run_json().stdout)
        self.assertTrue(payload["operator_review_only"])
        self.assertIn("human review only", payload["explicit_no_execution_statement"])
