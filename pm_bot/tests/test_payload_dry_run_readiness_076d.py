...
def test_present_076c_module_without_evidence_blocks_even_when_legacy_069a_exists(tmp_path: Path) -> None:
    _write_selected_candidate(tmp_path / "sources")
    _write_selected_token_verification(tmp_path / "sources")
    ...
    assert signer["bridge_present"] is True
    assert signer["module_available"] is True
    assert signer["status"] == "blocked_missing_signer_diagnostic_evidence"
...
def test_failed_076c_signer_evidence_reports_failed_review_status(tmp_path: Path) -> None:
    _write_selected_candidate(tmp_path / "sources")
    _write_selected_token_verification(tmp_path / "sources")
    _write_signer_evidence(tmp_path / "sources", ok=False)
    _write_payload_stack(tmp_path / "sources")
    _write_risk_stack(tmp_path / "sources")
    ...
    assert result["current_top_blocker"] == "blocked_signer_diagnostic_failed"
    assert signer["status"] == "blocked_signer_diagnostic_failed"
    assert signer["diagnostic_ok"] is False
    assert signer["bridge_present"] is True
    assert result["validation"]["valid"] is True
    _assert_required_false_flags(result)
