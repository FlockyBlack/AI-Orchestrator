...
REQUEST_CONTRACT_VERSION = "pmbot_local_supervised_live_readiness_dashboard_request.v1"
DASHBOARD_CONTRACT_VERSION = "pmbot_local_supervised_live_readiness_dashboard.v1"
LOCAL_RUN_MODE = "local_static_supervised_live_readiness_dashboard"
OPERATOR_REVIEW_STATUS = "pending_operator_review"
DASHBOARD_ROW_STATE = "ready_for_operator_review"
SAMPLE_DASHBOARD_PATH = "pm_bot/dashboard/samples/local_supervised_live_readiness_dashboard.fixture.json"
SAMPLE_OPERATOR_REPORT_PATH = "pm_bot/dashboard/samples/local_supervised_live_readiness_dashboard.fixture.md"
...
class SupervisedLiveReadinessDashboardValidationResult:
    valid: bool
    errors: tuple[str, ...]


class SupervisedLiveReadinessDashboardValidationError(ValueError):
...
def load_dashboard_request(path: str | Path) -> dict[str, Any]:
    ...
    if _is_network_like(normalized):
        raise SupervisedLiveReadinessDashboardValidationError(("request path must be local",))
    if _contains_path_traversal(normalized):
        raise SupervisedLiveReadinessDashboardValidationError(("request path must not use traversal",))
    return json.loads(Path(path).read_text(encoding="utf-8"))


def build_local_supervised_live_readiness_dashboard(request: Mapping[str, Any]) -> dict[str, Any]:
    validation = validate_dashboard_request(request)
    if not validation.valid:
        raise SupervisedLiveReadinessDashboardValidationError(validation.errors)

    queue_rows = [_build_queue_row(row) for row in request["queue_records"]]
    readiness_rows = [_build_readiness_row(row) for row in request["readiness_records"]]
    ...
    dashboard = {
        ...
        "summary_counts": _summary_counts(queue_rows, readiness_rows, validation_rows, []),
        "validation_status_summary": validation_rows,
        "warnings": [],
    }

    artifact_validation = validate_local_supervised_live_readiness_dashboard(dashboard)
    if not artifact_validation.valid:
        raise SupervisedLiveReadinessDashboardValidationError(artifact_validation.errors)
    return dashboard


def validate_dashboard_request(request: Mapping[str, Any]) -> SupervisedLiveReadinessDashboardValidationResult:
    errors: list[str] = []
    required_fields = (
        "contract_version",
        "dashboard_id",
    ...
    if request.get("scope") != "local_supervised_live_readiness_dashboard":
        errors.append("scope must be local_supervised_live_readiness_dashboard")
    ...
    for field_name in ("queue_records", "readiness_records", "validation_records"):
    ...
    errors.extend(_duplicate_id_errors("readiness_records", request.get("readiness_records"), "artifact_id"))
    ...
    return SupervisedLiveReadinessDashboardValidationResult(valid=not errors, errors=tuple(errors))


def validate_local_supervised_live_readiness_dashboard(
    dashboard: Mapping[str, Any],
) -> SupervisedLiveReadinessDashboardValidationResult:
    errors: list[str] = []
    required_fields = (
        "build_id",
        "contract_version",
    ...
    if dashboard.get("safety_boundaries") != LOCAL_ONLY_SAFETY_BOUNDARIES:
        errors.append("safety_boundaries must match the local-only supervised-live readiness dashboard boundary")

    queue_rows = dashboard.get("queue_summary")
    readiness_rows = dashboard.get("readiness_summary")
    ...
    if not isinstance(readiness_rows, list):
        errors.append("readiness_summary must be a list")
        readiness_rows = []
    ...
    _validate_output_rows("readiness_summary", readiness_rows, errors)
    ...
    if isinstance(summary_counts, Mapping):
        ...
        if dict(summary_counts) != expected_counts:
            errors.append("summary_counts must match supervised-live readiness dashboard rows")
    else:
...
def build_operator_report(dashboard: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Readiness artifacts: {dashboard['summary_counts']['readiness_artifacts']}",
        f"- Readiness rows: {dashboard['summary_counts']['readiness_rows']}",
        f"- Supporting artifacts: {dashboard['summary_counts']['supporting_artifacts']}",
        f"- Operator review checks: {dashboard['summary_counts']['operator_review_checks']}",
        f"- Validation commands: {dashboard['summary_counts']['validation_commands']}",
    ...
    for row in dashboard["readiness_summary"]:
        lines.append(
            f"- `{row['artifact_id']}`: type `{row['artifact_type']}`, rows {row['readiness_rows']}, "
            f"supporting artifacts {row['supporting_artifacts']}, review `{row['operator_review_status']}`, "
            f"fixture `{row['fixture_reference']}`"
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build a local PMBOT supervised-live readiness dashboard.")
    parser.add_argument("--request", required=True, help="Path to a local supervised-live readiness dashboard request JSON file.")
    parser.add_argument("--output-dashboard", required=True, help="Path for the output dashboard JSON.")
    parser.add_argument("--output-report", required=True, help="Path for the output dashboard Markdown.")
...
def _build_queue_row(row: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "record_id": f"queue_supervised_live_readiness.{row['task_id']}",
...
def _build_readiness_row(row: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "local_reference": _normalize_local_reference(row["local_reference"]),
        "notes": row.get("notes", ""),
        "operator_review_checks": int(row["operator_review_checks"]),
        "operator_review_status": row["operator_review_status"],
        "readiness_rows": int(row["readiness_rows"]),
        "record_id": f"supervised_live_readiness_dashboard.{row['artifact_id']}",
        "required_state": row["required_state"],
        "run_mode": row["run_mode"],
        "runner_state": DASHBOARD_ROW_STATE,
        "scope": row["scope"],
        "status_label": row["status_label"],
        "supporting_artifacts": int(row["supporting_artifacts"]),
        "validation_commands": int(row["validation_commands"]),
    }
...
def _summary_counts(
    ...
    readiness_rows: Sequence[Any],
    validation_rows: Sequence[Any],
    warnings: object,
) -> dict[str, int]:
    readiness_objects = [row for row in readiness_rows if isinstance(row, Mapping)]
    return {
        "operator_review_checks": _sum_integer_field(readiness_objects, "operator_review_checks"),
        "operator_review_pending_records": _count_pending(queue_rows, readiness_rows, validation_rows),
        "queue_records": len(queue_rows),
        "readiness_artifacts": len(readiness_rows),
        "readiness_rows": _sum_integer_field(readiness_objects, "readiness_rows"),
        "supporting_artifacts": _sum_integer_field(readiness_objects, "supporting_artifacts"),
        "validation_commands": _sum_integer_field(readiness_objects, "validation_commands"),
...
def _validate_local_reference(reference: str, field_path: str) -> list[str]:
    ...
    if _is_forbidden_reference(normalized):
        errors.append(f"{field_path} is outside the supervised-live readiness dashboard boundary")
    if not normalized.startswith(ALLOWED_LOCAL_REFERENCE_PREFIXES):
        errors.append(f"{field_path} must stay under allowed local supervised-live readiness dashboard paths")
    return errors
...
def _forbidden_term_errors(value: object, path: str) -> list[str]:
    if isinstance(value, Mapping):
        ...
        for key, nested_value in value.items():
            ...
            if _has_forbidden_dashboard_term(str(key)):
                errors.append(f"forbidden supervised-live readiness dashboard decision field detected at {key_path}")
    ...
    if isinstance(value, str) and _has_forbidden_dashboard_term(value):
        return [f"forbidden supervised-live readiness dashboard decision value detected at {path}"]
    return []
