...
RUNNER = ROOT / "pm_bot" / "quality" / "research_quality_scorecard.py"
EXPECTED_JSON = ROOT / "pm_bot" / "quality" / "expected_research_quality_scorecard.v1.json"
EXPECTED_MD = ROOT / "pm_bot" / "quality" / "expected_research_quality_scorecard.v1.md"
...
class ResearchQualityScorecardTests(unittest.TestCase):
    ...
    def test_score_keeps_determinism_and_safety_scores_high(self):
        payload = json.loads(_run_json().stdout)
        self.assertEqual(payload["score_by_section"]["determinism"], 100)
        self.assertEqual(payload["score_by_section"]["safety_boundary_clarity"], 100)
