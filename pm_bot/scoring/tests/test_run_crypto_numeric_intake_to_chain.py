...
RUNNER = ROOT / "pm_bot" / "scoring" / "run_crypto_numeric_intake_to_chain.py"
EXPECTED_JSON = ROOT / "pm_bot" / "scoring" / "expected_crypto_numeric_intake_to_chain.v1.json"
EXPECTED_MD = ROOT / "pm_bot" / "scoring" / "expected_crypto_numeric_intake_to_chain.v1.md"
...
class RunCryptoNumericIntakeToChainTests(unittest.TestCase):
    ...
    def test_intake_to_chain_summary(self):
        payload = json.loads(_run_json().stdout)
        self.assertEqual(payload["intake_summary"]["raw_markets"], 11)
        self.assertEqual(payload["intake_summary"]["normalized_supported"], 4)
        self.assertEqual(payload["intake_summary"]["rejected"], 7)
        self.assertEqual(payload["score_summary"]["markets_scored"], 4)
        self.assertEqual(payload["score_summary"]["paper_candidates"], 1)
        self.assertEqual(payload["score_summary"]["watchlist"], 1)
        self.assertEqual(payload["score_summary"]["rejected_after_scoring"], 2)
    ...
    def test_rejected_raw_markets_are_retained(self):
        payload = json.loads(_run_json().stdout)
        self.assertEqual(len(payload["rejected_raw_markets"]), 7)
        reason_codes = {row["reason_code"] for row in payload["rejected_raw_markets"]}
        self.assertEqual(
            reason_codes,
            {
    ...
    def test_safety_flags_remain_locked_down(self):
        ...
        for section in ("score_report", "review_table", "generated_paper_order_plan"):
            self.assertTrue(payload[section]["offline_only"])
            self.assertTrue(payload[section]["paper_only"])
            self.assertFalse(payload[section]["execution_allowed"])
            self.assertFalse(payload[section]["trading_allowed"])
