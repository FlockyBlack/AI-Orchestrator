...
SAFE_ASSIGNMENT_FLAGS = ("FORBIDDEN", "SAFE", "ALLOWED", "SCAN", "SOURCE_EXTENSIONS")
FORBIDDEN_PATTERNS = [
...
def _iter_target_files(root: Path):
    roots = [
        ...
        root / "pm_bot" / "paper",
        root / "pm_bot" / "risk",
        root / "pm_bot" / "accounting",
        root / "pm_bot" / "postmortem",
        root / "docs",
    ]
...
def _safe_assignment_spans(tree):
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Assign):
            ...
            name = node.target.id
            if any(flag in name for flag in SAFE_ASSIGNMENT_FLAGS):
                spans.append((node.lineno, node.end_lineno))
    return spans
...
def build_static_audit_report(root: Path):
    ...
    for path in _iter_target_files(root):
        scanned_files.append(str(path.relative_to(root)).replace("\\", "/"))
                                ...
                                if imported_name == spec["token"]:
                                    ...
                                    if key not in blocking_keys:
                                        ...
                                        blocking_findings.append(
                                            _finding(root, path, spec["token"], spec["category"], node.lineno, "runtime_import_detected")
                                        )
    ...
    return {
        "schema_version": "v2",
        "audit_id": "PMBOT-BATCH-003-STATIC-AUDIT",
        ...
        "checks": {
            "no_network_or_api": audit_passed and not (categories & {"network", "live_api"}),
            "no_live_polymarket_api": audit_passed and "live_api" not in categories,
            "no_wallet_or_pk": audit_passed and "wallet" not in categories,
            ...
            "no_orchestration_runner_imports": audit_passed and "runtime" not in categories,
...
def main(argv):
    if len(argv) != 0:
        print(json.dumps({"status": "invalid", "error": "usage: static_safety_audit_v2.py"}, separators=(",", ":")))
        return 2
