...
REQUIRED_DOCS = {
    "docs/PM_BOT_READONLY_FETCHER_IMPLEMENTATION_PLAN_V1.md",
    "docs/PM_BOT_READONLY_FETCHER_APPROVAL_CHECKLIST_V1.md",
    "docs/PM_BOT_READONLY_FETCHER_FAILURE_MODES_V1.md",
    "docs/PM_BOT_STAGE_SUMMARY_V9.md",
    "docs/PM_BOT_SAFE_BACKLOG_V9.json",
    "docs/PM_BOT_BATCH_008_RESULT.json",
}
...
def build_static_audit_report(root: Path):
    ...
    live_readonly_paths = []
    live_readonly_root = root / "pm_bot" / "live_readonly"
    if live_readonly_root.exists():
        live_readonly_paths.append("pm_bot/live_readonly")
        blocking_findings.append(
            {
                "file": "pm_bot/live_readonly",
                "token": "live_readonly_package_present",
                "category": "live_fetcher",
                "line": 1,
                "reason": "live_readonly_package_not_allowed",
            }
        )

    missing_required_docs = sorted(REQUIRED_DOCS - set(scanned_files))
    for relative in missing_required_docs:
        blocking_findings.append(
            {
                "file": relative,
                "token": "required_planning_artifact_missing",
                "category": "planning",
                "line": 1,
                "reason": "required_planning_artifact_missing",
            }
        )
    ...
    checks = {
        "batch_008_planning_only": audit_passed and "planning" not in categories,
        "no_live_fetcher_module_exists": audit_passed and "live_fetcher" not in categories,
        "no_live_readonly_executable_package_exists": audit_passed and not live_readonly_paths,
        ...
        "planning_docs_are_static_artifacts": audit_passed and not missing_required_docs,
    ...
    return {
        "schema_version": "v7",
        "audit_id": "PMBOT-BATCH-008-STATIC-AUDIT",
        "audit_passed": audit_passed,
        "scanned_file_count": len(scanned_files),
        "scanned_files": scanned_files,
        "boundary_runtime_sources": sorted(boundary_runtime_sources),
        "live_readonly_paths": live_readonly_paths,
...
def main(argv):
    if len(argv) != 0:
        print(json.dumps({"status": "invalid", "error": "usage: static_safety_audit_v7.py"}, separators=(",", ":")))
        return 2
