...
SCHEMA_VERSION = "manual_dossier_draft_validation_result.v1"
MARKDOWN_VERSION = "manual_dossier_draft_validation_report.v1"
ROOT = Path(__file__).resolve().parents[2]
DEFAULT_DRAFT_RECORDS = ROOT / "pm_bot" / "research" / "manual_dossier_drafts_fixture.v1.json"
DEFAULT_DOSSIER_SKELETONS = ROOT / "pm_bot" / "research" / "dossier_draft_skeletons.v1.json"
...
DEFAULT_JSON_RESULT = ROOT / "pm_bot" / "research" / "manual_dossier_draft_validation_result.v1.json"
DEFAULT_MARKDOWN_REPORT = ROOT / "pm_bot" / "research" / "manual_dossier_draft_validation_report.v1.md"
DEFAULT_EXPECTED_JSON_RESULT = ROOT / "pm_bot" / "research" / "expected_manual_dossier_draft_validation_result.v1.json"
...
def _parse_args(argv):
    parser = argparse.ArgumentParser(description="Validate deterministic offline PMBOT manual dossier draft attempts.")
            ...
            if _field_tokens(key_text) & PROHIBITED_FIELD_TOKENS:
                findings.append(
                    _error(
                        ...
                        path,
                        "Trading, execution, recommendation, bet, stake, size, price, scoring, signal, probability, EV, wallet, private-key, side, buy, and sell fields are prohibited in manual dossier drafts.",
                    )
                )
...
def _load_skeleton_context(path):
    ...
    if not isinstance(skeletons, list):
        raise ValueError("dossier skeleton payload must contain dossier_draft_skeletons list")
    ...
    return payload, skeleton_by_market_id, immutable_fields


def _load_merged_market_ids(payload):
    packets = payload.get("packets")
    if not isinstance(packets, list):
        raise ValueError("merged packet payload must contain packets list")
    return {
...
def _load_review_market_ids(payload):
    market_ids = set()
...
def _draft_record_entries(payload):
    ...
    if not isinstance(payload, dict):
        return None, _error("draft_records_payload_not_object_or_list", "draft_records", "Manual dossier draft JSON must be an object or a list.")
...
def _validate_record(record, index, skeleton_market_ids, known_market_ids, immutable_fields):
    market_id = _draft_market_id(record, index)
    errors = []
    if not isinstance(record, dict):
        return market_id, [_error("draft_record_not_object", f"draft_records[{index}]", "Each manual dossier draft record must be an object.")]
    ...
    if not _non_empty_text(record.get("market_id")):
        _append(errors, "missing_draft_market_id", "market_id", "Manual dossier draft must include a non-empty market_id.")
    elif market_id not in skeleton_market_ids:
        if market_id in known_market_ids:
            _append(errors, "non_skeleton_market_id", "market_id", "Manual dossier draft market_id was not exported as a RESEARCH-012 dossier draft skeleton.")
        else:
            _append(errors, "unknown_market_id", "market_id", "Manual dossier draft market_id is absent from the local skeleton, review, and merged-packet artifacts.")
    ...
    for field in sorted(set(record) - ALLOWED_DRAFT_FIELDS):
        if field in immutable_fields:
            ...
            _append(errors, f"unexpected_draft_field:{field}", field, f"{field} is not an allowed manual dossier draft field.")
    ...
    if "next_manual_action" not in record:
        ...
        _append(errors, "invalid_next_manual_action", "next_manual_action", "next_manual_action is not in the allowed operational action set.")
...
def build_manual_dossier_draft_validation_result(
    ...
    skeleton_payload, skeleton_by_market_id, immutable_fields = _load_skeleton_context(dossier_skeletons_path)
    ...
    known_market_ids = set(skeleton_market_ids)
    known_market_ids.update(_load_merged_market_ids(merged_payload))
    known_market_ids.update(_load_review_market_ids(review_payload))
    ...
    for index, record in enumerate(entries or []):
        market_id, errors = _validate_record(record, index, skeleton_market_ids, known_market_ids, immutable_fields)
        if errors:
    ...
    accepted_records.sort(key=lambda item: (item["market_id"], item["record_index"]))
    rejected_records.sort(key=lambda item: (item["market_id"], item["record_index"]))
    ...
    return {
        ...
        "skeleton_market_ids": sorted(skeleton_market_ids),
...
def write_manual_dossier_draft_validation_artifacts(
    draft_records_path=DEFAULT_DRAFT_RECORDS,
    dossier_skeletons_path=DEFAULT_DOSSIER_SKELETONS,
    review_records_result_path=DEFAULT_REVIEW_RECORDS_RESULT,
    merged_packets_path=DEFAULT_MERGED_PACKETS,
    ...
    markdown_output_path = _resolve_path(markdown_output_path)
    expected_json_output_path = _resolve_path(expected_json_output_path)
    result = build_manual_dossier_draft_validation_result(
...
def main(argv):
    ...
    summary = write_manual_dossier_draft_validation_artifacts(
