...
EXPORTER = ROOT / "pm_bot" / "research" / "export_selected_ingest_final_dossier_drafts.py"
REVIEW_PACK = ROOT / "pm_bot" / "research" / "selected_ingest_dossier_human_review_pack.v1.json"
REVIEW_RECORDS_RESULT = (
    ROOT / "pm_bot" / "research" / "selected_ingest_dossier_human_review_records_result.v1.json"
)
JSON_OUTPUT = ROOT / "pm_bot" / "research" / "selected_ingest_final_dossier_drafts.v1.json"
MARKDOWN_OUTPUT = ROOT / "pm_bot" / "research" / "selected_ingest_final_dossier_drafts.v1.md"
EXPECTED_JSON_OUTPUT = ROOT / "pm_bot" / "research" / "expected_selected_ingest_final_dossier_drafts.v1.json"
...
def _load_module():
    spec = importlib.util.spec_from_file_location("selected_ingest_final_dossier_draft_exporter", EXPORTER)
...
class SelectedIngestFinalDossierDraftExportTests(unittest.TestCase):
    ...
    def test_only_approved_for_final_dossier_draft_selected_ingest_records_are_exported(self):
        export = _load_export()
        ...
        self.assertEqual(drafts[0]["market_id"], "824952")
    ...
    def test_non_approved_records_are_skipped_without_context_lookup(self):
        ...
        watch_record["market_id"] = "692258"
        watch_record["human_review_outcome"] = "watch_only"
        watch_record["reviewer_notes"] = "Watch-only selected-ingest record must not produce a draft."
        revision_record = dict(approved_record)
        revision_record["record_index"] = 8
        revision_record["market_id"] = "597964"
        revision_record["human_review_outcome"] = "needs_draft_revision"
        revision_record["reviewer_notes"] = "Revision selected-ingest record must not produce a draft."
        custom_review_result = {
            "schema_version": "custom_selected_ingest_review_result.v1",
            "review_summary": {
                "review_records_read": 3,
                "review_records_accepted": 3,
                "review_records_rejected": 0,
                "approved_for_final_dossier_draft": 1,
                "needs_draft_revision": 1,
                "rejected_for_research_quality": 0,
                "watch_only": 1,
            },
            "accepted_human_review_records": [watch_record, revision_record, approved_record],
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            review_result_path.write_text(
                json.dumps(custom_review_result, indent=2, ensure_ascii=True) + "\n",
                encoding="utf-8",
            )
            export = module.build_selected_ingest_final_dossier_drafts_export(
        ...
        self.assertEqual(export["export_summary"]["review_records_skipped"], 2)
    ...
    def test_evidence_and_review_content_are_copied_structurally_without_truth_inference(self):
        ...
        self.assertEqual(draft["title_question"], pack["title_question"])
        self.assertEqual(draft["event_id"], pack["event_id"])
        self.assertEqual(draft["event_title"], pack["event_title"])
        self.assertEqual(draft["volume"], pack["volume"])
        ...
        self.assertEqual(draft["source_ingest_artifacts"], pack["source_ingest_artifacts"])
        self.assertEqual(
            draft["final_draft_sections"]["human_review_summary"]["human_review_notes"],
            review_record["reviewer_notes"],
        )
    ...
    def test_exporter_has_no_live_fetcher_runtime_or_downstream_automation_terms(self):
