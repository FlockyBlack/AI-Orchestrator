...
SCHEMA_VERSION = "dossier_human_review_records_result.v1"
MARKDOWN_VERSION = "dossier_human_review_records_report.v1"
ROOT = Path(__file__).resolve().parents[2]
DEFAULT_REVIEW_RECORDS = ROOT / "pm_bot" / "research" / "dossier_human_review_records_fixture.v1.json"
DEFAULT_REVIEW_PACK = ROOT / "pm_bot" / "research" / "dossier_human_review_pack.v1.json"
DEFAULT_VALIDATION_RESULT = ROOT / "pm_bot" / "research" / "manual_dossier_draft_validation_result.v1.json"
DEFAULT_DOSSIER_SKELETONS = ROOT / "pm_bot" / "research" / "dossier_draft_skeletons.v1.json"
DEFAULT_JSON_RESULT = ROOT / "pm_bot" / "research" / "dossier_human_review_records_result.v1.json"
DEFAULT_MARKDOWN_REPORT = ROOT / "pm_bot" / "research" / "dossier_human_review_records_report.v1.md"
DEFAULT_EXPECTED_JSON_RESULT = ROOT / "pm_bot" / "research" / "expected_dossier_human_review_records_result.v1.json"
...
def _parse_args(argv):
    parser = argparse.ArgumentParser(description="Validate deterministic offline PMBOT dossier human review records.")
            ...
            if key_text not in PROHIBITED_FIELD_EXCEPTIONS and _field_tokens(key_text) & PROHIBITED_FIELD_TOKENS:
                findings.append(
                    _error(
                        ...
                        path,
                        "Trading, execution, recommendation, bet, stake, size, price, scoring, signal, probability, EV, wallet, private-key, side, buy, sell, and market-decision fields are prohibited in dossier human review records.",
                    )
                )
...
def _review_record_entries(payload):
    ...
    if not isinstance(payload, dict):
        return None, _error("review_records_payload_not_object_or_list", "review_records", "Human review records JSON must be an object or a list.")
...
def _load_review_pack_context(path):
    ...
    if not isinstance(packs, list):
        raise ValueError("dossier human review pack payload must contain human_review_packs list")
    ...
    return payload, pack_by_market_id, immutable_fields
...
def _validate_record(record, index, review_pack_market_ids, known_market_ids, immutable_fields):
    ...
    if not _non_empty_text(record.get("market_id")):
        ...
        if market_id in known_market_ids:
            _append(errors, "non_review_pack_market_id", "market_id", "Human review record market_id is not present in dossier_human_review_pack.v1.json.")
        else:
            _append(errors, "unknown_market_id", "market_id", "Human review record market_id is absent from the local dossier review pack, draft validation, and skeleton artifacts.")

    for field in sorted(set(record) - ALLOWED_REVIEW_RECORD_FIELDS):
        if field in immutable_fields:
            _append(errors, f"immutable_review_pack_field_override:{field}", field, f"{field} is immutable review pack content and cannot be supplied by a human review record.")
        else:
            _append(errors, f"unexpected_human_review_field:{field}", field, f"{field} is not an allowed human review record field.")
...
def build_dossier_human_review_record_result(
    ...
    review_pack_payload, pack_by_market_id, immutable_fields = _load_review_pack_context(review_pack_path)
    ...
    known_market_ids = set(review_pack_market_ids)
    known_market_ids.update(_market_ids_from_records(validation_payload, ("accepted_draft_records", "rejected_draft_records")))
    known_market_ids.update(_market_ids_from_skeletons(skeleton_payload))
    ...
    for index, record in enumerate(entries or []):
        market_id, errors = _validate_record(record, index, review_pack_market_ids, known_market_ids, immutable_fields)
...
def write_dossier_human_review_record_artifacts(
    review_records_path=DEFAULT_REVIEW_RECORDS,
    review_pack_path=DEFAULT_REVIEW_PACK,
    validation_result_path=DEFAULT_VALIDATION_RESULT,
    dossier_skeletons_path=DEFAULT_DOSSIER_SKELETONS,
    ...
    markdown_output_path = _resolve_path(markdown_output_path)
    expected_json_output_path = _resolve_path(expected_json_output_path)
    result = build_dossier_human_review_record_result(
...
def main(argv):
    ...
    summary = write_dossier_human_review_record_artifacts(
