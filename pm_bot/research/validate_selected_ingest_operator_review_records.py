...
SCHEMA_VERSION = "selected_ingest_operator_review_records_result.v1"
MARKDOWN_VERSION = "selected_ingest_operator_review_records_report.v1"
ROOT = Path(__file__).resolve().parents[2]
DEFAULT_REVIEW_RECORDS = ROOT / "pm_bot" / "research" / "selected_ingest_operator_review_records_fixture.v1.json"
...
DEFAULT_JSON_RESULT = ROOT / "pm_bot" / "research" / "selected_ingest_operator_review_records_result.v1.json"
DEFAULT_MARKDOWN_REPORT = ROOT / "pm_bot" / "research" / "selected_ingest_operator_review_records_report.v1.md"
DEFAULT_EXPECTED_JSON_RESULT = ROOT / "pm_bot" / "research" / "expected_selected_ingest_operator_review_records_result.v1.json"
...
def _parse_args(argv):
    parser = argparse.ArgumentParser(
        description="Validate deterministic offline selected-ingest operator review records."
    )
            ...
            if key_text not in PROHIBITED_FIELD_EXCEPTIONS and _field_tokens(key_text) & PROHIBITED_FIELD_TOKENS:
                findings.append(
                    _error(
                        ...
                        path,
                        "Trading, execution, recommendation, bet, stake, size, price, scoring, probability, expected value, side, buy/sell, wallet, private-key, and market decision fields are prohibited.",
                    )
                )
...
def _load_selected_queue(queue_path):
    payload = _load_json(queue_path)
    if not isinstance(payload, dict):
        raise ValueError("selected ingest operator review queue payload must be a JSON object")
    if tuple(str(market_id) for market_id in payload.get("selected_market_ids", ())) != SELECTED_MARKET_IDS:
        raise ValueError("selected ingest operator review queue has unexpected selected_market_ids")
    if not isinstance(payload.get("groups"), dict):
        raise ValueError("selected ingest operator review queue must contain groups")
    return payload


def _load_selected_merged_packets(merged_packets_path):
...
def _review_record_entries(review_records_payload):
    ...
    if not isinstance(review_records_payload, dict):
        return None, _error(
            ...
            "review_records",
            "Review records JSON must be an object or a list.",
        )
...
def _validate_record(record, index, market_contexts, immutable_fields, seen_market_ids):
    ...
    if not _non_empty_text(record.get("market_id")):
        ...
        _append(
            ...
            "unknown_market_id",
            "market_id",
            "Review record market_id is not present in the selected-ingest operator review queue.",
        )
    ...
    for field in sorted(set(record) - ALLOWED_REVIEW_RECORD_FIELDS):
        if field in immutable_fields:
            _append(
                errors,
                f"immutable_packet_field_override:{field}",
                field,
                f"{field} is immutable packet or queue content and cannot be supplied by an operator review record.",
            )
        else:
    ...
    if "requested_followup_information" in record:
        requested_followup_information = _validate_string_list(
            record["requested_followup_information"],
            "requested_followup_information",
            errors,
        )
...
def _selected_sort_key(record):
    ...
    return (market_index, market_id, record.get("review_outcome", ""), record.get("review_status", ""))


def build_selected_ingest_operator_review_record_result(
    ...
    queue_payload = _load_selected_queue(operator_review_queue_path)
    merged_payload, packets = _load_selected_merged_packets(merged_packets_path)
    ...
    if payload_error is not None:
        rejected_records.append(_normalized_rejected_record({}, "payload", None, [payload_error]))
        entries = []
...
def write_selected_ingest_operator_review_record_artifacts(
    review_records_path=DEFAULT_REVIEW_RECORDS,
    operator_review_queue_path=DEFAULT_OPERATOR_REVIEW_QUEUE,
    merged_packets_path=DEFAULT_MERGED_PACKETS,
    json_output_path=DEFAULT_JSON_RESULT,
    ...
    result = build_selected_ingest_operator_review_record_result(
...
def main(argv):
    ...
    summary = write_selected_ingest_operator_review_record_artifacts(
