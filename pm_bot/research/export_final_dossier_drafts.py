...
TASK_ID = "PMBOT-RESEARCH-016-FINAL-DOSSIER-DRAFT-EXPORT"
SCHEMA_VERSION = "final_dossier_drafts.v1"
MARKDOWN_VERSION = "final_dossier_drafts_markdown.v1"
ROOT = Path(__file__).resolve().parents[2]
DEFAULT_REVIEW_PACK = ROOT / "pm_bot" / "research" / "dossier_human_review_pack.v1.json"
DEFAULT_REVIEW_RECORDS_RESULT = ROOT / "pm_bot" / "research" / "dossier_human_review_records_result.v1.json"
...
DEFAULT_JSON_OUTPUT = ROOT / "pm_bot" / "research" / "final_dossier_drafts.v1.json"
DEFAULT_MARKDOWN_OUTPUT = ROOT / "pm_bot" / "research" / "final_dossier_drafts.v1.md"
DEFAULT_EXPECTED_JSON_OUTPUT = ROOT / "pm_bot" / "research" / "expected_final_dossier_drafts.v1.json"
...
def _parse_args(argv):
    parser = argparse.ArgumentParser(description="Export deterministic offline PMBOT final dossier drafts for human reading.")
...
def _human_review_pack_by_market_id(payload):
    return _by_market_id(_records_list(payload, "human_review_packs"), "human review pack")
...
def _validation_records_by_market_id(payload):
    ...
    return _by_market_id(accepted, "accepted draft validation record")


def _skeletons_by_market_id(payload):
    return _by_market_id(_records_list(payload, "dossier_draft_skeletons"), "dossier skeleton")


def _merged_packets_by_market_id(payload):
    return _by_market_id(_records_list(payload, "packets"), "merged research packet")
...
def build_final_dossier_drafts_export(
    ...
    drafts.sort(key=lambda item: (item["market_id"], item["title_question"]))
...
def write_final_dossier_draft_artifacts(
    review_pack_path=DEFAULT_REVIEW_PACK,
    review_records_result_path=DEFAULT_REVIEW_RECORDS_RESULT,
    validation_result_path=DEFAULT_VALIDATION_RESULT,
    dossier_skeletons_path=DEFAULT_DOSSIER_SKELETONS,
    ...
    markdown_output_path = _resolve_path(markdown_output_path)
    expected_json_output_path = _resolve_path(expected_json_output_path)
    export_payload = build_final_dossier_drafts_export(
...
def main(argv):
    ...
    summary = write_final_dossier_draft_artifacts(
