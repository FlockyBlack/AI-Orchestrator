...
SCHEMA_VERSION = "dossier_human_review_pack.v1"
MARKDOWN_VERSION = "dossier_human_review_pack_markdown.v1"
...
DEFAULT_JSON_OUTPUT = ROOT / "pm_bot" / "research" / "dossier_human_review_pack.v1.json"
DEFAULT_MARKDOWN_OUTPUT = ROOT / "pm_bot" / "research" / "dossier_human_review_pack.v1.md"
DEFAULT_EXPECTED_JSON_OUTPUT = ROOT / "pm_bot" / "research" / "expected_dossier_human_review_pack.v1.json"
...
def _parse_args(argv):
    parser = argparse.ArgumentParser(description="Export deterministic offline PMBOT dossier human-review packs.")
...
def _records_list(payload, field):
    ...
    if not isinstance(records, list):
        raise ValueError(f"validation result must contain {field} list")
    ...
    for skeleton in skeletons:
        if isinstance(skeleton, dict) and _clean_text(skeleton.get("market_id")):
            ...
            if market_id in by_market_id:
                raise ValueError(f"duplicate skeleton market_id: {market_id}")
...
def _review_market_ids(payload):
    ...
    if not isinstance(records, list):
        raise ValueError("operator review records result must contain accepted_review_records list")
    return {
...
def _assert_pack_fields(pack_items):
    ...
    for item in pack_items:
        if list(item) != expected:
            raise ValueError(f"human review pack field sequence mismatch for {item.get('market_id', '')}")
...
def build_dossier_human_review_pack_export(
    ...
    accepted_records = _records_list(validation_payload, "accepted_draft_records")
    rejected_records = _records_list(validation_payload, "rejected_draft_records")
    ...
    pack_items = []
    for record in sorted(accepted_records, key=lambda item: (_clean_text(item.get("market_id")), item.get("record_index", 0))):
        if _clean_text(record.get("draft_status")) != READY_DRAFT_STATUS:
            continue
        market_id = _clean_text(record.get("market_id"))
        if market_id not in skeleton_by_market_id:
            raise ValueError(f"ready accepted draft is missing dossier skeleton context: {market_id}")
        if market_id not in merged_market_ids:
            raise ValueError(f"ready accepted draft is missing merged packet context: {market_id}")
        if market_id not in review_market_ids:
            raise ValueError(f"ready accepted draft is missing operator review context: {market_id}")
        pack_items.append(_pack_item(record, skeleton_by_market_id[market_id]))

    pack_items.sort(key=lambda item: (item["market_id"], item["title_question"]))
...
def write_dossier_human_review_pack_artifacts(
    validation_result_path=DEFAULT_VALIDATION_RESULT,
    dossier_skeletons_path=DEFAULT_DOSSIER_SKELETONS,
    merged_packets_path=DEFAULT_MERGED_PACKETS,
    review_records_result_path=DEFAULT_REVIEW_RECORDS_RESULT,
    ...
    markdown_output_path = _resolve_path(markdown_output_path)
    expected_json_output_path = _resolve_path(expected_json_output_path)
    export_payload = build_dossier_human_review_pack_export(
...
def main(argv):
    ...
    summary = write_dossier_human_review_pack_artifacts(
