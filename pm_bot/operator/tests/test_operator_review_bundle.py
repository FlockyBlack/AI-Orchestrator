...
RUNNER = ROOT / "pm_bot" / "operator" / "build_operator_review_bundle.py"
EXPECTED_JSON = ROOT / "pm_bot" / "operator" / "expected_operator_review_bundle.v1.json"
EXPECTED_MD = ROOT / "pm_bot" / "operator" / "expected_operator_review_bundle.v1.md"
...
class OperatorReviewBundleTests(unittest.TestCase):
    ...
    def test_bundle_keeps_review_only_scope(self):
        payload = json.loads(_run_json().stdout)
        self.assertTrue(payload["generated_from_local_artifacts"])
        self.assertTrue(payload["operator_review_only"])
        self.assertIn("does not authorize", payload["explicit_no_execution_statement"])
