...
from pm_bot.trading_core.runtime_credential_visibility_077c import (
    ...
    render_runtime_credential_visibility_cli_summary,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run PMBOT runtime credential visibility diagnostic 077C."
    )
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Required. Checks explicit environment variable visibility and writes redacted no-live artifacts.",
    )
    parser.add_argument(
        "--artifacts-dir",
        "--artifact-dir",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.dry_run is not True:
        raise SystemExit("runtime credential visibility diagnostic requires --dry-run; live execution is blocked")
    ...
    if args.json:
        ...
        print(render_runtime_credential_visibility_cli_summary(result))
    return 0
