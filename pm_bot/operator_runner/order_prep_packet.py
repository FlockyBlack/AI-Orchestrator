...
from pm_bot.trading_core.order_prep_packet import (
    fail_closed_for_forbidden_flags,
    render_order_prep_packet_cli_summary,
    run_order_prep_packet,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run PMBOT order prep packet 072A.")
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Required. Builds a local non-executable no-submit order prep packet.",
    )
    parser.add_argument(
        "--artifact-root",
        default="",
    ...
    parser.add_argument(
        ...
        dest="artifacts_dir",
        default="",
        help="Optional output directory. Defaults to the 072A order prep packet artifact directory.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.dry_run is not True:
        raise SystemExit("order prep packet requires --dry-run; live execution is blocked")
    ...
    if args.json:
        ...
        print(render_order_prep_packet_cli_summary(result))
    return 0
