...
from pm_bot.trading_core.selected_token_verification_bridge import (
    ...
    render_selected_token_verification_cli_summary,
    run_selected_token_verification_bridge,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run PMBOT selected token verification bridge 076A.")
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Required. Verifies local selected token evidence without live execution.",
    )
    parser.add_argument(
        "--artifact-root",
        default="",
    ...
    parser.add_argument(
        ...
        default="",
        help="Optional explicit local 075D selected candidate artifact path.",
    )
    parser.add_argument(
        "--operator-token-selection-packet-path",
        default="",
        help="Optional explicit local 073B operator token selection artifact path.",
    )
    parser.add_argument(
        "--artifacts-dir",
        "--artifact-dir",
        dest="artifacts_dir",
        default="",
        help="Optional output directory. Defaults to the 076A verification bridge artifact directory.",
    )
    parser.add_argument("--json", action="store_true", help="Print latest verification status JSON.")
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.dry_run is not True:
        raise SystemExit("selected token verification bridge requires --dry-run; live execution is blocked")
    ...
    if args.json:
        ...
        print(render_selected_token_verification_cli_summary(result))
    return 0
