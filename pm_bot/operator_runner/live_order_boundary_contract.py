...
from pm_bot.trading_core.live_order_boundary_contract import (
    ...
    render_live_order_boundary_contract_cli_summary,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run PMBOT live order boundary contract 065B.")
    ...
    parser.add_argument("--dry-run", action="store_true", help="Required. Generates interface-only artifacts.")
    parser.add_argument(
        "--artifacts-dir",
        "--artifact-dir",
        dest="artifacts_dir",
        default="",
        help="Optional output directory. Defaults to the 065B live order boundary artifact directory.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.dry_run is not True:
        raise SystemExit("live order boundary contract requires --dry-run; executable live boundaries are blocked")
    ...
    if args.json:
        ...
        print(render_live_order_boundary_contract_cli_summary(status))
    return 0
