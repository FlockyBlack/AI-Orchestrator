...
from pm_bot.trading_core.selected_candidate_artifact import (
    ...
    render_selected_candidate_artifact_cli_summary,
    run_selected_candidate_artifact,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run PMBOT selected candidate artifact 075D.")
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Required. Builds a local review-only selected candidate artifact.",
    )
    parser.add_argument(
        "--candidate-index",
        default="",
        help="Optional zero-based source-backed candidate_index from the local 073B candidate artifact.",
    )
    parser.add_argument(
        "--artifact-root",
        default="",
    ...
    parser.add_argument(
        ...
        dest="artifacts_dir",
        default="",
        help="Optional output directory. Defaults to the 075D artifact directory.",
    )
    parser.add_argument("--json", action="store_true", help="Print latest artifact status JSON.")
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.dry_run is not True:
        raise SystemExit("selected candidate artifact requires --dry-run; live execution is blocked")
    ...
    if args.json:
        ...
        print(render_selected_candidate_artifact_cli_summary(result))
    return 0
