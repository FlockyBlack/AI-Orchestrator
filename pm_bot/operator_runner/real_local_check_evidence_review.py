...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run PMBOT real local-check evidence review 074A.")
    ...
    parser.add_argument(
        ...
        dest="artifacts_dir",
        default="",
        help="Optional output directory. Defaults to the 074A evidence review artifact directory.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.dry_run is not True:
        raise SystemExit("real local-check evidence review requires --dry-run; live execution is blocked")
    ...
    if args.json:
        ...
        print(render_real_local_check_evidence_review_cli_summary(status))
    return 0
