...
from pm_bot.trading_core.explicit_live_credentials_readiness_gate import (
    ...
    render_explicit_live_credentials_readiness_gate_cli_summary,
    run_explicit_live_credentials_readiness_gate,
)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run PMBOT explicit live credentials readiness gate 064.")
    ...
    parser.add_argument("--dry-run", action="store_true", help="Required. Generates review-only artifacts.")
    parser.add_argument(
        "--artifacts-dir",
        "--artifact-dir",
        dest="artifacts_dir",
        default="",
        help="Optional output directory. Defaults to the 064 explicit live credentials readiness artifact directory.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.dry_run is not True:
        raise SystemExit("explicit live credentials readiness gate requires --dry-run; live execution is blocked")
    ...
    if args.json:
        ...
        print(render_explicit_live_credentials_readiness_gate_cli_summary(status))
    return 0
