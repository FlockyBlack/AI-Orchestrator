...
RUNNER = ROOT / "pm_bot" / "validation" / "false_positive_prevention_report.py"
EXPECTED_JSON = ROOT / "pm_bot" / "validation" / "expected_false_positive_prevention_report.v1.json"
EXPECTED_MD = ROOT / "pm_bot" / "validation" / "expected_false_positive_prevention_report.v1.md"
...
class FalsePositivePreventionReportTests(unittest.TestCase):
    ...
    def test_report_confirms_no_false_positives(self):
        payload = json.loads(_run_json().stdout)
        self.assertEqual(payload["false_positives_count"], 0)
        self.assertEqual(payload["high_risk_false_positives_count"], 0)
