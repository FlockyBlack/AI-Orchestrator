...
STATUS_MISSING_SIGNER_DIAGNOSTIC_EVIDENCE = "blocked_missing_signer_diagnostic_evidence"
STATUS_MISSING_SIGNER_DIAGNOSTIC_EVIDENCE_BRIDGE = "missing_signer_diagnostic_evidence_bridge_076c"
STATUS_SIGNER_DIAGNOSTIC_EVIDENCE_OK = "signer_diagnostic_evidence_ok_for_payload_dry_run"
...
def _summarize_signer_diagnostic_evidence(
    ...
    *,
    ...
    bridge_present = source_available or module_available
    safety_flags_ok = source_available and _source_false_flags_ok(payload, UNSAFE_TRUE_FIELDS)
    diagnostic_ok = (
        source_available
        and safety_flags_ok
        and (
            payload.get("signer_diagnostic_evidence_ok_for_payload_dry_run") is True
            or payload.get("signer_diagnostic_evidence_ok") is True
            or payload.get("diagnostic_ok") is True
            or status
            in {
                "diagnostic_ok",
                "signer_diagnostic_evidence_ok",
                "signer_diagnostic_evidence_bridge_ok",
                STATUS_SIGNER_DIAGNOSTIC_EVIDENCE_OK,
            }
        )
    )
    if not source_available:
        status = (
            STATUS_MISSING_SIGNER_DIAGNOSTIC_EVIDENCE
            if bridge_present
            else STATUS_MISSING_SIGNER_DIAGNOSTIC_EVIDENCE_BRIDGE
        )
    return {
        "status": status,
        ...
        "safety_flags_ok": safety_flags_ok,
        "signer_diagnostic_evidence_ok_for_payload_dry_run": diagnostic_ok,
        "source_path": clean_text(source.get("path")),
...
def _build_blockers(
    ...
    *,
    ...
    if selected_candidate.get("ready") is True and selected_verification.get("verified") is True and signer.get("diagnostic_ok") is not True:
        signer_status = clean_text(signer.get("status"))
        blocker_id = (
            STATUS_BLOCKED_SIGNER_DIAGNOSTIC_FAILED
            if signer_status == STATUS_BLOCKED_SIGNER_DIAGNOSTIC_FAILED
            else STATUS_BLOCKED_SIGNER_DIAGNOSTIC_NOT_OK
        )
        reason = (
            "076C signer diagnostic evidence bridge is present but reports blocked_signer_diagnostic_failed."
            if blocker_id == STATUS_BLOCKED_SIGNER_DIAGNOSTIC_FAILED
            else "076C signer diagnostic evidence bridge is missing or not diagnostic_ok; legacy 069A evidence is not treated as the 076C bridge."
        )
        blockers.append(
            _blocker(
                blocker_id,
...
def _status_for_components(component_statuses: Mapping[str, Mapping[str, Any]]) -> str:
    ...
    if signer.get("diagnostic_ok") is not True:
        if clean_text(signer.get("status")) == STATUS_BLOCKED_SIGNER_DIAGNOSTIC_FAILED:
...
def _operator_summary(status: str) -> str:
    ...
    if status == STATUS_BLOCKED_SIGNER_DIAGNOSTIC_FAILED:
        return "Payload dry-run readiness is blocked because 076C signer diagnostic evidence reports a failed signer diagnostic."
    if status == STATUS_BLOCKED_SIGNED_PAYLOAD_DRY_RUN_NOT_READY:
