...
- current top blocker: `blocked_signer_diagnostic_failed`
- next recommended safe command: `python -m pm_bot.operator_runner.signer_diagnostic_evidence_bridge --market BTC --strategy tiny-momentum --dry-run`
- operator summary: Payload dry-run readiness is blocked because 076C signer diagnostic evidence reports a failed signer diagnostic.
...
- `signer_diagnostic_evidence` status=`blocked_signer_diagnostic_failed` ready=False
- `payload_dry_run` status=`selected_token_payload_readiness_not_ready:blocked_signer_diagnostic_failed` ready=False
- `risk_engine` status=`blocked_risk_engine_or_final_reducer` ready=False

## Final Blockers

- `blocked_signer_diagnostic_failed` - 076C signer diagnostic evidence bridge is present but reports blocked_signer_diagnostic_failed.
...
- `signer_diagnostic_evidence_bridge_076c` available=True status=`blocked_signer_diagnostic_failed` path=`pm_bot/trading_core/artifacts/signer_diagnostic_evidence_bridge_076c/latest_signer_diagnostic_evidence_076c_status.json`
- `guarded_signer_diagnostic_smoke_069a` available=True status=`blocked_missing_private_key` path=`pm_bot/trading_core/artifacts/guarded_signer_diagnostic_smoke_069a/latest_guarded_signer_diagnostic_status_069a.json`
- `selected_token_payload_readiness_gate_073c` available=True status=`blocked_signer_diagnostic_failed` path=`pm_bot/trading_core/artifacts/selected_token_payload_readiness_gate_073c/latest_selected_token_payload_readiness_status_073c.json`
